...
def pip_mastermix(df_map, outFH, mmvolume=13.1, mmtube=1, liq_cls='Water Free Single'):
    ...
    * for 1:n_dispense
    ...
    MD.DestRackLabel = df_map.loc[:,'TECAN_dest_labware']
    MD.DestPositions = df_map.loc[:,'TECAN_dest_location']
    MD.Volume = mmvolume
...
def pip_nonbarcode_primer(df_map, outFH, volume, tube, liq_cls='Water Free Single'):
    ...
    for i in range(df_map.shape[0]):
        ...
        disp.RackLabel = df_map.loc[i,'TECAN_dest_labware']
        disp.Position = df_map.loc[i,'TECAN_dest_location']
        disp.Volume = volume
...
def pip_primers(df_map, outFH, fp_volume=0, rp_volume=0,
    ...
    for i in range(df_map.shape[0]):
        # aspiration
        asp = Fluent.aspirate()
        asp.RackLabel = df_map.loc[i,'TECAN_primer_labware']
        asp.Position = df_map.loc[i,'TECAN_primer_location']
...
def pip_samples(df_map, outFH, liq_cls='Water Free Single'):
    ...
    for i in range(df_map.shape[0]):
        # aspiration
        asp = Fluent.aspirate()
        asp.RackLabel = df_map.loc[i,'TECAN_sample_labware']
        asp.Position = df_map.loc[i,'TECAN_sample_location']
