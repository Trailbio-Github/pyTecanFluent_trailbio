...
def reorder_384well(df, reorder_col):
    ...
    df = df.drop('TECAN_sort_IS_EVEN', axis=1)
...
def pip_samples(df_setup, outFH, liq_cls='Water Contact Wet Single'):
    ...
    for i in range(df.shape[0]):
        # aspiration
        asp = Fluent.aspirate()
        asp.RackLabel = df.loc[i,'sample labware']
        asp.Position = df.loc[i,'sample location']
        asp.Volume = round(df.loc[i,'sample volume'], 2)
        asp.LiquidClass = liq_cls
        outFH.write(asp.cmd() + '\n')

        # dispensing
        disp = Fluent.dispense()
        disp.RackLabel = df.loc[i,'dest_labware']
        disp.Position = df.loc[i,'dest_location']
        disp.Volume = round(df.loc[i,'sample volume'], 2)
...
def pip_water(df_setup, outFH, src_labware, src_start=1, 
    ...
    for i in range(df.shape[0]):
        ...
        asp.Volume = round(df.loc[i,'water volume'], 2)
        asp.LiquidClass = liq_cls
        outFH.write(asp.cmd() + '\n')
        ...
        disp.RackLabel = df.loc[i,'dest_labware']
        disp.Position = df.loc[i,'dest_location']
        disp.Volume = round(df.loc[i,'water volume'], 2)
        disp.LiquidClass = liq_cls
        outFH.write(disp.cmd() + '\n')
